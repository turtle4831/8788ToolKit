...
import config
from Wrappers import TurtleMotor, TurtleSubsystem
...
class Drivetrain(TurtleSubsystem):
    ...
    def DriveCommand(self,x=0,y=0,z=0):
        if not config.isAuto:
    ...
    def init(self):
        ...
        commands2.CommandScheduler.getInstance().setDefaultCommand(
            self,
            self.DriveCommand(config.autox, config.autoy, config.autoz)
        )


    def periodic(self):
        ommands2.CommandScheduler.getInstance().setDefaultCommand(
            self,
            self.DriveCommand(config.autox, config.autoy, config.autoz)
        )
